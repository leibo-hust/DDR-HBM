...
void TraceBasedCPU::ClockTick() {
    memory_system_.ClockTick();
    if (!trace_file_.eof()) {       // 一直到trace文件的最末尾
        if (get_next_) {
            get_next_ = false;
            trace_file_ >> trans_;          // 每次读取trace文件中的一行，到trans_（事物）中
        }
        if (trans_.added_cycle <= clk_) {
            get_next_ = memory_system_.WillAcceptTransaction(trans_.addr,
                                                             trans_.is_write);      // 这里判断是否应该执行这个transaction
            if (get_next_) {
                memory_system_.AddTransaction(trans_.addr, trans_.is_write);        // 如果应该执行，那么这里就由memory_system负责执行事务
                // memory_system会调用dram_system.AddTranscation();
            }
        }
    }
    clk_++;
