...
std::pair<uint64_t, int> Controller::ReturnDoneTrans(uint64_t clk) {
    // 返回队列中第一个元素的迭代器
    auto it = return_queue_.begin();        // vector<Transaction> return_queue_;
    while (it != return_queue_.end()) {     // 进行迭代
        if (clk >= it->complete_cycle) {        // 如果当前时钟，大于当前事物的完成周期
            if (it->is_write) {         // 如果是写操作
            ...
            it = return_queue_.erase(it);       // 将处理完的擦除
            return pair;        // 返回组成的pair（地址，是否写）
        } else {
            ++it;
        }
    }
    return std::make_pair(-1, -1);      // 最后返回-1， -1
}
...
// JedecDRAMSystem::AddTransaction调用这里，根据地址和是否写判断
bool Controller::WillAcceptTransaction(uint64_t hex_addr, bool is_write) const {
    if (is_unified_queue_) {       // is_unified_queue定义在config类中，默认初始值为false
        return unified_queue_.size() < unified_queue_.capacity();       // 将uunified队列大小和队列的容量进行对比，如果没超过容量则可以接受事物
    } else if (!is_write) {         // 如果不是unified_queue，并且当前事物不是写操作
        return read_queue_.size() < read_queue_.capacity();     // 如果不是写，那么就是读。判断当前读队列大小和其容量大小
    } else {
        return write_buffer_.size() < write_buffer_.capacity();     // 如果是写，那么就判断写缓冲的大小和其实际容量。如果超过了容量，那么肯定不允许写
    }
}
